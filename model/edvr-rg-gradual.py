...
def make_model(args):
    return EDVR(args.n_feats, args.n_frames, args.n_groups,
                args.front_RBs, args.n_resblocks, args.n_resgroups, args.n_cagroups)
...
class TSA_Fusion(nn.Module):
    ...
    def __init__(self, nf=64, nframes=5, center=2):
        ...
        for i in range(nframes-1):
            fusion.append(nn.Conv2d((nframes-i) * nf, (nframes-i-1) * nf, 1, 1, bias=True))
            fusion.append(nn.LeakyReLU(negative_slope=0.1))
        # self.fea_fusion = nn.Conv2d(nframes * nf, nf, 1, 1, bias=True)
        self.fea_fusion = nn.Sequential(*fusion)
