...
def make_model(args):
    return EDVR(args.n_feats, args.n_frames, args.n_groups,
                args.channel_growth, args.n_densegroups, args.n_resblocks, args.n_resgroups, args.n_cagroups)
...
class ResidualGroup(nn.Module):
    def __init__(self, n_feat, n_resblocks, ca=True):
        super(ResidualGroup, self).__init__()
        modules_body = []
        if ca:
            modules_body = [ResidualBlock_noBN_CA(n_feat) for _ in range(n_resblocks)]
        else:
            modules_body = [ResidualBlock_noBN(n_feat) for _ in range(n_resblocks)]

        modules_body.append(nn.Conv2d(n_feat, n_feat, 3, 1, 1, bias=True))
...
class DenseBlock(nn.Module):
    def __init__(self, nf, channel_growth):
        super(DenseBlock, self).__init__()
        self.conv1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(nf, channel_growth, 3, 1, 1, bias=True)
...
class DenseGroup(nn.Module):
    def __init__(self, nf, channel_growth, n_denseblocks):
        super(DenseGroup, self).__init__()
        modules_body = []
        for i in range(n_denseblocks):
            modules_body.append(DenseBlock(nf+i*channel_growth, channel_growth))
        modules_body.append(nn.Conv2d(nf+n_denseblocks*channel_growth, nf, 1, 1, 0, bias=True))
...
class DenseExtract(nn.Module):
    def __init__(self, nf, channel_growth, n_densegroups, n_denseblocks=5):
        super(DenseExtract, self).__init__()
        self.groups = nn.ModuleList()
        for i in range(n_densegroups):
            self.groups.append(DenseGroup(nf, channel_growth, n_denseblocks))
        self.fusion = nn.Sequential(*[nn.Conv2d(n_densegroups * nf, nf, 1, 1, 0),
                                      nn.Conv2d(nf, nf, 3, 1, 1)])

    def forward(self, x):
        group_out = []
        for i in range(len(self.groups)):
            group_out.append(self.groups[i](x))
        out = self.fusion(torch.cat(group_out, dim=1))
        return x + out
...
class EDVR(nn.Module):
    def __init__(self,
        ...
        self.feature_extraction = DenseExtract(nf, channel_growth, n_densegroups)
        ...
        #### reconstruction
        recon1 = [ResidualGroup(nf, n_resblocks, ca=False) for i in range(n_resgroups)]
        self.recon_trunk = nn.Sequential(*recon1)
        # self.recon_trunk = make_layer(ResidualBlock_noBN_CA_f, back_RBs)
        recon2 = [ResidualGroup(nf, n_resblocks, ca=True) for i in range(n_cagroups)]
        self.recon_trunk2 = nn.Sequential(*recon2)
        #### upsampling
